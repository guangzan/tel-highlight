...
function highlightNum(tel: Tel, highlight: Highlight) {
  if (typeof tel == 'number') tel = String(tel)
  if (typeof highlight === 'number') highlight = String(highlight)

  if (
    highlight === null ||
    highlight === undefined ||
    highlight === '' ||
    tel.length < highlight.length
  ) {
    return [
      {
        num: tel,
...
function joinSeparator(highlightList: HighlightList, separator: Separator) {
  ...
  const newHighlightList: HighlightList = []
  ...
  const hasHighlight = () =>
    newHighlightList.some((i) => i.isHighlight === true)

  if (!hasHighlight()) {
    const _newHighlightList = []
    let item: HighlightListItem = { num: '', isHighlight: false }
